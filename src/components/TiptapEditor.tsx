...
export default function TiptapEditor({ content, onChange }: TiptapEditorProps) {
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        ...
        heading: { HTMLAttributes: { class: 'font-black italic tracking-tighter uppercase leading-tight mt-8 mb-4 hover:text-primary transition-colors' } },
        blockquote: { HTMLAttributes: { class: 'border-l-4 border-primary pl-6 py-4 my-8 bg-primary/5 rounded-r-3xl italic text-xl font-medium text-gray-700' } },
      }),
      Image.configure({
        allowBase64: true,
        HTMLAttributes: {
  ...
  const addVideo = useCallback(() => videoInputRef.current?.click(), []);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success) {
        if (file.type.startsWith('image/')) editor.chain().focus().setImage({ src: data.url }).run();
        else if (file.type.startsWith('video/')) editor.chain().focus().setVideo({ src: data.url }).run();
      }
    } catch (err) {
        ...
        {editor && (
          ...
          >
            <div className="bg-white border shadow-2xl rounded-2xl flex items-center p-2 gap-2 backdrop-blur-xl bg-white/90">
              <button onClick={() => editor.chain().focus().setTextAlign('left').run()} className={`p-3 rounded-xl transition-all ${editor.isActive({ textAlign: 'left' }) ? 'bg-primary text-white shadow-lg' : 'hover:bg-gray-100 text-gray-500'}`}><AlignLeft className="w-5 h-5" /></button>
              <button onClick={() => editor.chain().focus().setTextAlign('center').run()} className={`p-3 rounded-xl transition-all ${editor.isActive({ textAlign: 'center' }) ? 'bg-primary text-white shadow-lg' : 'hover:bg-gray-100 text-gray-500'}`}><AlignCenter className="w-5 h-5" /></button>
              <button onClick={() => editor.chain().focus().setTextAlign('right').run()} className={`p-3 rounded-xl transition-all ${editor.isActive({ textAlign: 'right' }) ? 'bg-primary text-white shadow-lg' : 'hover:bg-gray-100 text-gray-500'}`}><AlignRight className="w-5 h-5" /></button>
...
function ToolbarButton({ onClick, icon, label, active = false }: { onClick: () => void, icon: React.ReactNode, label: string, active?: boolean }) {
  return (
    <button type="button" onClick={onClick} className={`flex flex-col items-center gap-1.5 transition-all group ${active ? 'text-primary scale-110' : 'text-gray-600 hover:text-primary hover:scale-110'}`}>
      <div className={`p-4 rounded-[1.25rem] transition-all shadow-sm ${active ? 'bg-primary text-white shadow-primary/30' : 'bg-white group-hover:bg-primary/5'}`}>{icon}</div>
...
function FormatButton({ onClick, active, label }: { onClick: () => void, active: boolean, label: string }) {
  return (
    <button type="button" onClick={onClick} className={`w-10 h-10 rounded-xl text-xs font-black transition-all ${active ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-transparent text-gray-500 hover:bg-white hover:text-primary'}`}>{label}</button>
  );
}

function IconButton({ onClick, active, icon }: { onClick: () => void, active: boolean, icon: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} className={`p-2.5 rounded-xl transition-all ${active ? 'bg-primary/10 text-primary shadow-sm ring-1 ring-primary/20' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`}>{icon}</button>
  );
}
